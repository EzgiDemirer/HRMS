...
class DepartementsController extends Controller
{
    ...
    public function store(DepartementsRequest $request)
    {
        try {
            ...
            return redirect()->route('departements.index')->with(['success' => 'Data inserted successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('departements.index')->with(['error' => 'Unable to access the requested data!']);
        }
    ...
    public function update($id, DepartementsRequest $request)
    {
        try {
            ...
            $CheckExsists = Departement::select("id")
            ...
            return redirect()->route('departements.index')->with(['success' => 'Data updated successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with(['error' => 'Sorry, something went wrong: ' . $ex->getMessage()])->withInput();
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('departements.index')->with(['success' => 'Data deleted successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with(['error' => 'Sorry, something went wrong: ' . $ex->getMessage()])->withInput();
        }
