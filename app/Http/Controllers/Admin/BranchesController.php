...
class BranchesController extends Controller
{
    ...
    public function store(BrachesRequest $requst)
    {
        try {
            ...
            if (!empty($checkExsists)) {
                return redirect()->back()->with(['error' => 'Branch name already exists!'])->withInput();
            }
            ...
            return redirect()->route('branches.index')->with(['success' => 'Data inserted successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('branches.index')->with(['error' => 'Unable to access the requested data']);
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('branches.index')->with(['success' => 'Data deleted successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->route('branches.index')->with(['error' => 'Sorry, something went wrong: ' . $ex->getMessage()]);
        }
    }
}