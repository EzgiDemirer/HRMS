...
return redirect()->back()->with(['error' => 'This data already exists'])->withInput();
}
...
return redirect()->route('ShiftsTypes.index')->with(['success' => 'Data saved successfully']);
...
if (empty($data)) {
return redirect()->route('ShiftsTypes.index')->with(['error' => 'Unable to access the requested data']);
}
...
$checkExsitsData = Shifts_type::select("id")
->where('type', '=', $request->type)
->where('from_time', '=', $request->from_time)
->where('to_time', '=', $request->to_time)
->where('total_hour', '=', $request->total_hour)
->where('id', '!=', $id)
->first();
...
return redirect()->route('ShiftsTypes.index')->with(['success' => 'Data updated successfully']);
...
return redirect()->route('ShiftsTypes.index')->with(['success' => 'Data deleted successfully']);
