...
return redirect()->route('finance_calender.index')->with(['success' => 'Data inserted successfully']);
...
if ($data['is_open'] != 0) {
return redirect()->back()->with(['error' => 'The fiscal year cannot be edited in this case']);
}
...
if ($data['is_open'] != 0) {
return redirect()->back()->with(['error' => 'The fiscal year cannot be edited in this case'])->withInput();
}
...
if($validator->fails()){
return redirect()->back()->with(['error' => 'Fiscal year name already exists'])->withInput();
}
...
return redirect()->route('finance_calender.index')->with(['success' => 'Data updated successfully']);
...
if ($data['is_open'] != 0) {
return redirect()->back()->with(['error' => 'The fiscal year cannot be deleted in this case']);
}
...
return redirect()->route('finance_calender.index')->with(['success' => 'Data deleted successfully']);
...
if ($data['is_open'] != 0) {
return redirect()->back()->with(['error' => 'The fiscal year cannot be opened in this case']);
}

$CheckDataOpenCounter = Finance_calender::where(['is_open' => 1])->count();

if ($CheckDataOpenCounter>0) {
return redirect()->back()->with(['error' => 'There is already an open fiscal year']);
}
