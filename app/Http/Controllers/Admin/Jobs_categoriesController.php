...
class Jobs_categoriesController extends Controller
{
    ...
    public function store(Jobs_categoriesRequest $request)
    {
        try {
            ...
            if (!empty($CheckExsists)) {
                return redirect()->back()->with(['error' => 'Job title already exists!'])->withInput();
            }
            ...
            return redirect()->route('jobs_categories.index')->with(['success' => 'Data added successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('jobs_categories.index')->with(['error' => 'Unable to access the requested data']);
        }
    ...
    public function update($id, Jobs_categoriesRequest $request)
    {
        ...
        try {
            $CheckExsists = jobs_categorie::select("id")
            ...
            return redirect()->route('jobs_categories.index')->with(['success' => 'Data updated successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with(['error' => 'Sorry, something went wrong: ' . $ex->getMessage()])->withInput();
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('jobs_categories.index')->with(['success' => 'Data deleted successfully']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->route('jobs_categories.index')->with(['error' => 'Sorry, something went wrong: ' . $ex->getMessage()]);
        }
    }
}